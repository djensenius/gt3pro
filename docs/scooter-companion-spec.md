# Scooter Companion — iOS App Specification

> A native Swift iOS app for the GT3 Pro that provides live ride telemetry via Dynamic Island / Live Activity, automatic background data sync, and upload to a self-hosted monitoring stack.
...
**Scooter Companion** is a personal-use iOS app that connects to a SuperScooter GT3 Pro over Bluetooth Low Energy (BLE), reads telemetry and diagnostic data, and uploads it to a self-hosted monitoring stack (InfluxDB → Grafana) via the fluxhaus-server API.
...
| Property | Value |
|---|---|
| Model | SuperScooter GT3 (also GT3 Pro) |
...
centralManager = CBCentralManager(
    ...
    queue: bleQueue,
    options: [
        CBCentralManagerOptionRestoreIdentifierKey: "ScooterCompanionCentral"
    ]
)
```
...
**Recovering password from official mobility app** (one-time, avoids re-pairing):
...
```
Scooter Companion ── HTTPS POST ──► fluxhaus-server
                                      │
                          ┌───────────┴───────────┐
                          │                       │
                          ▼                       ▼
...
**Authentication**: OIDC — same `AuthManager` as the FluxHaus iOS app. The Scooter Companion authenticates using the shared OIDC provider and includes the bearer token in all API requests.
...
> FluxHaus uses Peach/Mauve as primary accents; Scooter Companion uses Sky/Sapphire to visually distinguish the apps while maintaining the shared Catppuccin foundation.
...
- **Scooter Companion**: Sumi-e scooter silhouette
...
### Option B: Recover Password from the Official Mobility App (no button press needed)
...
<string>Scooter Companion connects to your GT3 Pro to read ride telemetry and sync data.</string>

<key>NSBluetoothPeripheralUsageDescription</key>
<string>Scooter Companion needs Bluetooth to communicate with your GT3 Pro.</string>

<!-- Location -->
<key>NSLocationWhenInUseUsageDescription</key>
<string>Scooter Companion records your GPS route during rides.</string>
<key>NSLocationAlwaysAndWhenInUseUsageDescription</key>
<string>Scooter Companion needs background location to track rides when your phone is in your pocket.</string>

<!-- Health -->
<key>NSHealthShareUsageDescription</key>
<string>Scooter Companion reads heart rate data from your Apple Watch during rides.</string>
<key>NSHealthUpdateUsageDescription</key>
<string>Scooter Companion saves ride workouts to Apple Health.</string>

<!-- Motion -->
<key>NSMotionUsageDescription</key>
<string>Scooter Companion uses motion sensors to detect road surface quality.</string>
...
├── ScooterCompanion/                       # Main app target
│   ├── App/
│   │   ├── ScooterCompanionApp.swift       # @main entry, SwiftData container setup
...
│       └── Logger.swift
├── ScooterCompanionWidgets/                # Widget Extension target (Live Activity)
│   ├── GT3RideAttributes.swift         # Shared ActivityAttributes definition
│   ├── GT3LiveActivity.swift           # Dynamic Island + Lock Screen layouts
│   └── ScooterCompanionWidgetsBundle.swift
├── ScooterCompanionWatch/                  # watchOS app target
│   ├── ScooterCompanionWatchApp.swift      # @main entry for Watch app
...
**Mitigation**: Write extensive unit tests comparing Swift output against known Java outputs for multiple seeds. Alternatively, recover the password from the official mobility app backup to bypass SET_PWD entirely.
...
### Concurrent BLE Access with the Official Mobility App

**Risk**: If the official mobility app is also installed and tries to connect, two apps fighting over the same BLE peripheral will cause connection failures.
...
> *"Scooter Companion connects to your GT3 Pro over Bluetooth to read speed, battery, and ride data in real time."*
...
> *"Scooter Companion records your GPS route during rides so you can see where you've been and explore ride data on a map."*
...
> *"To track rides when your phone is in your pocket or the app is in the background, Scooter Companion needs 'Always' location access. Your location is only recorded during active rides — never at other times."*

- If denied, GPS fields in telemetry samples are `nil`. Rides still work without GPS.

**Step 3 — HealthKit (optional)**

> *"If you have an Apple Watch, Scooter Companion can record your heart rate and calories during rides — like a cycling workout."*
...
> *"Scooter Companion uses your phone's motion sensors to detect road surface quality — bumpy roads, smooth pavement, and everything in between."*
...
> *"Scooter Companion can notify you about ride summaries, charging complete, and connection events."*
